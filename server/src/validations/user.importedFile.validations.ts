...
const typeCheckers = {
  ...
  alphabetic: (str: string) => alphabeticsRegex.test(str),
  boolean: (str: string) => validBooleanValues.has(str),
  date: (str: string) => str !== "" && dateTimeRegex.test(str),
  integer: (str: string, rawValue: any, isExcel: boolean) => {
    if (isExcel && typeof rawValue !== "number") return false;
    return integerRegex.test(str);
  },
  float: (str: string, rawValue: any, isExcel: boolean) => {
    if (isExcel && typeof rawValue !== "number") return false;
...
export const extractDependencyColumns = (columnConfig: Record<string, any>) => {
  ...
  Object.values(columnConfig).forEach((col: any) => {
    ...
    const mainColumn = col.name; // ✅ normalize

    Object.keys(dependency).forEach((key) => {
      // ❌ skip main column safely
      if (key === mainColumn) return;
    ...
    if (rule.cell_contains && rule.cell_contains_value) {
      ...
      if (!regexCache.has(regexString)) {
        ...
        if (match) {
          ...
          const flags = match[2];
          regexCache.set(regexString, new RegExp(pattern, flags));
        } else {
          regexCache.set(regexString, new RegExp(regexString, "u"));
        }
      }
      rule.cellContainsRegex = regexCache.get(regexString);
    }
    if (rule.cell_start_with) {
...
export const validateRow = (
  ...
  for (let i = 0; i < headers.length; i++) {
    ...
    if (rawValue === null || rawValue === undefined) {
      strValue = "";
    } else if (primaryType === "date" && typeof rawValue === "number") {
      const jsDate = excelDateToJSDate(rawValue);
      strValue = formatDate(jsDate);
    } else if (typeof rawValue === "object") {
      if (rawValue.value !== undefined) {
        strValue =
          typeof rawValue.value === "boolean"
            ? rawValue.value
              ? "true"
              : "false"
            : String(rawValue.value);
      } else if (rawValue.formula) {
        strValue = String(rawValue.formula).toLowerCase();
      } else if (rawValue.richText) {
        strValue = rawValue.richText.map((t: any) => t.text).join("");
      } else if (rawValue.text) {
        strValue = String(rawValue.text);
      } else if (rawValue.result) {
        strValue = String(rawValue.result);
      } else {
        strValue = "";
      }
    } else {
    ...
    if (rule) {
      ...
      if (rule.cellContainsRegex) {
        // Skip regex test for empty values (already handled by required check)
        if (strValue !== "") {
          // Reset lastIndex only if regex is global (avoid unnecessary reset)
          if (rule.cellContainsRegex.global) {
            rule.cellContainsRegex.lastIndex = 0;
          }

          if (!rule.cellContainsRegex.test(strValue)) {
            columnStat.regex_pattern_error_count++;
            markInvalid();
            pushError({ columnStat, ruleKey: "regex", rowNumber });
            // Remove errorMsg creation if not used (commented out anyway)
          }
        }
      }

      if (rule.data_types_new.length > 0) {
        const dataTypes = rule.data_types_new;
        let isValid = false;

        // ✅ Manual if-else chain is fastest for common types
        for (let i = 0; i < dataTypes.length; i++) {
          ...
          if (type === "integer") {
            if (
              (!isExcel || typeof rawValue === "number") &&
          ...
          if (type === "boolean") {
            if (validBooleanValues.has(strValue)) {
              isValid = true;
              break;
            }
            continue;
...
const pushError = ({
  ...
  if (!columnStat.errorRowSets) {
    columnStat.errorRowSets = {};
  }
  if (!columnStat.errorRowSets[ruleKey]) {
    columnStat.errorRowSets[ruleKey] = new Set();
    columnStat.error_rows[ruleKey] = [];
  }
  if (!columnStat.errorRowSets[ruleKey].has(rowNumber)) {
    columnStat.errorRowSets[ruleKey].add(rowNumber);
